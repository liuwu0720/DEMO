	...
	function hoysures(id)
		{
			...
			if(confirm("请假是否确认")) 
			{
            	window.location="${pageContext.request.contextPath }/EmployholidayServlet/updateEmpSatByID?id="+id;
...
<form action="" method="post"></form></div></div>
   ...
   <table style="margin-top: 100px;">
						<tr align="center">
							<td colspan="4" align="center"> --员工请假--</td>
						</tr>
						<tr>
						...
						<c:forEach items="${holidaylist}" var="hoday">
							<tr>
								...
								<td>${hoday[3]}</td>
								<td>${hoday[6]}</td>
								<td><fmt:formatDate value="${hoday[4]}" pattern="yyyy-MM-dd HH:mm:ss"/> </td>
								<td><fmt:formatDate value="${hoday[5]}" pattern="yyyy-MM-dd HH:mm:ss"/></td>
