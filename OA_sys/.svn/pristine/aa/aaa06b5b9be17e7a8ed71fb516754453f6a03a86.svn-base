	...
	function delete_add(uid)
		{
		 ...
		 {
		  URL="${pageContext.request.contextPath }/UploadServlet/del?id="+uid;
		  window.location=URL;
		 }
		}
...
<dl class="notice clearfix">
  ...
  <dd>文件管理 &gt; 个人文件管理</dd>   
   ...
   <ul>
     ...
     <li class="btn_turn"><span>${page }/${pages }页</span><div><a href="UploadServlet/getSelfFile?recordid=${recordid }&page=${pages }"><i class="icon_next"></i></a><a href="UploadServlet/getSelfFile?recordid=${recordid }&page=1"><i class="icon_pre"></i></a></div></li>
...
<c:forEach items="${sm}" var="sm">
   <ul class="usr_info_lit clearfix">
    ...
    <li class="deals" >
    <a href="javascript:;" onclick="delete_add('${sm.lineid }');return false;"><i class="icon_del"></i>删除</a>
    <a href="${pageContext.request.contextPath }/UploadServlet/dowfilebyname?filename=${sm.filename }"><i class="icon_read"></i>下载</a>
    <a href="<%=path %>/oa_upload/sharefile.jsp?fileid=${sm.lineid}&filename=${sm.filename }"><i class="icon_corr"></i>共享</a>
